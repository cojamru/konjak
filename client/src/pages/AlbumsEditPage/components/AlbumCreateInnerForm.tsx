/* eslint-disable react/no-array-index-key */
/* eslint-disable no-prototype-builtins */

import { DeleteOutlined, PlusOutlined } from '@ant-design/icons';
import { Button, Form } from 'antd';
import { FieldArray, FormikProps } from 'formik';

import { TextField, DatePickerField, TextAreaField } from 'src/components';

import { AlbumCreateFormValuesType } from '../AlbumEditPageTypes';

import style from './AlbumEditForm.module.scss';

export const CreateAlbumInnerForm = (props: FormikProps<AlbumCreateFormValuesType>) => {
  const { values } = props;
  const { handleSubmit } = props;

  return (
    <Form className={style.editAlbumForm} labelCol={{ span: 4 }}>
      <Form.Item required label="Название">
        <TextField type="text" name="title" placeholder="Название" />
      </Form.Item>

      <Form.Item required label="Описание">
        <TextAreaField rows={6} type="textArea" name="description" placeholder="Описание" />
      </Form.Item>

      <Form.Item required label="slug">
        <TextField type="text" name="slug" placeholder="slug" />
      </Form.Item>

      <Form.Item required label="Дата релиза">
        <DatePickerField allowClear={false} name="release_date" />
      </Form.Item>

      <Form.Item className={style.editAlbumForm__fieldArray} label="Артисты">
        <FieldArray
          name="artists"
          render={arrayHelpers => (
            <div>
              {values.artists?.map((link, key) => {
                return (
                  <div data-justify-content="start" className={style.editAlbumForm__fieldArray__field} key={key}>
                    <div>
                      <TextField type="text" name={`artists[${key}].nickname`} placeholder="Никнейм" />
                    </div>

                    {values.artists.length !== 1 && (
                      <Button htmlType="button" onClick={() => arrayHelpers.remove(key)} danger>
                        <DeleteOutlined />
                      </Button>
                    )}

                    {values.artists.length - 1 === key && (
                      <Button onClick={() => arrayHelpers.push({ nickname: '' })}>
                        <PlusOutlined />
                      </Button>
                    )}
                  </div>
                );
              })}
              {!values.artists.length && (
                <Button onClick={() => arrayHelpers.push({ nickname: '' })}>
                  <PlusOutlined />
                </Button>
              )}
            </div>
          )}
        />
      </Form.Item>

      <Form.Item className={style.editAlbumForm__fieldArray} label="Ссылки">
        <FieldArray
          name="links"
          render={arrayHelpers => (
            <div>
              {values.links?.map((link, key) => {
                return (
                  <div
                    data-justify-content="space-between"
                    className={style.editAlbumForm__fieldArray__field}
                    key={key}
                  >
                    <div>
                      <TextField type="text" name={`links[${key}].title`} placeholder="Название" />
                    </div>
                    <div>
                      <TextField type="text" name={`links[${key}].url`} placeholder="URL" />
                    </div>

                    <Button htmlType="button" onClick={() => arrayHelpers.remove(key)} danger>
                      <DeleteOutlined />
                    </Button>
                  </div>
                );
              })}
              <Button onClick={() => arrayHelpers.push({ title: '', url: '' })}>
                <PlusOutlined />
              </Button>
            </div>
          )}
        />
      </Form.Item>

      <div className={style.editAlbumForm__actions}>
        <Button type="primary" onClick={() => handleSubmit()} htmlType="submit">
          Создать
        </Button>
      </div>
    </Form>
  );
};
